...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="status">
        <option value="1" {{ (isset($customer->status) && $customer->status == 1) ? 'selected' : '' }}>Active</option>
        <option value="0" {{ (isset($customer->status) && $customer->status == 0) ? 'selected' : '' }}>InActive</option>
    </select>
