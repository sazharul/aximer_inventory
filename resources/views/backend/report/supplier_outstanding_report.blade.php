...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">Supplier Outstanding Report</div>
                    <div class="card-body">
                        <form method="GET" action="{{ url('/supplier-outstanding-report') }}" accept-charset="UTF-8"
                                ...
                                <div class="col">
                                    <div class="form-group {{ $errors->has('category_id') ? 'has-error' : ''}}" style="display: inline-block; width: 200px;">
                                        ...
                                        <select class="form-control" name="supplier">
                                            @php
                                                $categories = \App\Models\Supplier::where('status', 1)->get();
                                            @endphp
                                            <option value="All">All</option>
                                            @foreach($categories as $item)
                                                <option
                                                    value="{{ $item->name }}" {{ (request('supplier') == $item->name) ? 'selected' : '' }}>{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach($supplier as $item)
                                        @php
                                            $sale = \App\Models\Purchase::where('supplier_id', $item->id)->pluck('id');
                                            $sale_invoice = \App\Models\PurchaseInvoice::where(function ($query) use ($sale) {
                                                    if (isset($sale)) {
                                                        $query->whereIn('purchase_id', $sale);
                                                    }
                                                })
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $total_due }}</td>
                                            <td>
                                                <a href="{{ route('supplier_invoice_list', $item->id) }}" class="btn btn-primary btn-sm">All Sale Invoice</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
